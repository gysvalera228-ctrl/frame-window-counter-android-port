...
// 生成二维 window 预设键：格式为 "if_window"，例如 "1_5" 或 "2_5"
inline std::string makeWindowPresetKey(int ifCount, double val) {
    if (ifCount < 1) ifCount = 1;
    return std::to_string(ifCount) + "_" + formatWindowVal(val);
}

...
namespace FileIO {
    ...
    void exportFWC() {
        ...
        async::spawn(file::pick(file::PickMode::SaveFile, file::FilePickOptions {
            .filters = {
                file::FilePickOptions::Filter { .description = "Frame Window Counter", .files = { "*.fwc" } },
                file::FilePickOptions::Filter { .description = "NANDL Calculator JSON", .files = { "*.json" } },
            }
        }), [exportList = std::move(exportList)](Result<std::optional<std::filesystem::path>> result) mutable {
            if (!result.isOk()) {
                auto msg = result.unwrapErr();
                FLAlertLayer::create("Error", fmt::format("Unable to select output file: {}", msg), "OK")->show();
                return;
            }
            auto selected = std::move(result).unwrap();
            if (!selected.has_value()) return;

            auto path = selected.value();
            auto ext = path.extension().string();
            std::transform(ext.begin(), ext.end(), ext.begin(), [](unsigned char c) { return static_cast<char>(std::tolower(c)); });
            bool isJson = ext == ".json";

            std::ofstream f(path, isJson ? std::ios::out : std::ios::binary);
            if (!f) {
                FLAlertLayer::create("Error", "Failed to save file.", "OK")->show();
                return;
            }
                ...
                for (const auto& act : exportList) {
                    ...
                    if (k <= 1) {
                        arr.push_back(matjson::makeObject({
                            {"input", inputCounter++}, {"timePosition", act.frame},
                            {"frameWindow", fw}, {"isPlayer2", act.isPlayer2}
                        }));
                    } else {
                        double w_main = std::max(fw - (k - 1.0) / k, 1.0 / k);
                        double w_sub = 1.0 / k;
                        arr.push_back(matjson::makeObject({
                            {"input", inputCounter++}, {"timePosition", base_frame},
                            {"frameWindow", w_main}, {"isPlayer2", act.isPlayer2}
                        }));
                        for (int sub = 1; sub < k; ++sub) {
                            double sub_frame = base_frame + static_cast<double>(sub) / static_cast<double>(k);
                            arr.push_back(matjson::makeObject({
                                {"input", inputCounter++}, {"timePosition", sub_frame},
                                {"frameWindow", w_sub}, {"isPlayer2", act.isPlayer2}
                            }));
                        }
                    }
                }
                auto rootObject = matjson::makeObject({
                    {"format", "nandl-calculator"}, {"version", 1},
                    {"settings", matjson::makeObject({
                        {"gameFps", g_macroFps}, {"windowFps", g_macroFps},
                        {"respawnSeconds", 0}, {"timeUnit", "frames"}
                    })},
                    {"frameWindows", arr}
                });
                f << rootObject.dump(2);
            ...
            f.close();
            FLAlertLayer::create("Success", "Successfully exported file!", "OK")->show();
        });
    }
    ...
    void importReplay(std::function<void()> onSuccessCallback) {
        ...
        async::spawn(file::pick(file::PickMode::OpenFile, file::FilePickOptions {
            .filters = { file::FilePickOptions::Filter {
                .description = "Supported Frame Window files",
                .files = { "*.fwc", "*.json", "*.gdr", "*.gdr2", "*.slc" },
            }}
        }), [onSuccessCallback = std::move(onSuccessCallback)](Result<std::optional<std::filesystem::path>> result) mutable {
            if (!result.isOk()) {
                FLAlertLayer::create("Error", fmt::format("Unable to select file: {}", result.unwrapErr()), "OK")->show();
                return;
            }
            auto selected = std::move(result).unwrap();
            if (!selected.has_value()) return;
